      ...
      <div className="container">
        ...
        <div className={menu ? "sidebar" : "sidebar hidden-2"} id="nav-links">
          ...
          <p onClick={() => handleParaClick("dashboard")}
            className={activePara === "dashboard" ? "active" : ""} to="/Dashboard" id="sidebar-item">
            <img src={Dashboard_V} />
            &emsp;Dashboard
          </p>
          ...
          <div className={empClick ? "ques hidden" : "ques"} id="ques">
            {/* Add Questions link */}
            <p onClick={() => handleParaClick('addQuestions')}
              className={activePara === 'addQuestions' ? 'active' : ''} id="sidebar-item" to="/AddQuestion">Add Questions</p>
            {/* Question bank link */}
            <p onClick={() => handleParaClick("questionbank")}
              className={activePara === "questionbank" ? "active" : ""} id="sidebar-item" to="/QuestionBank">Question bank</p>
          </div>
          {/* Jobseeker link */}
          <p onClick={() => handleParaClick("jobseeker")}
            className={activePara === "jobseeker" ? "active" : ""} id="sidebar-item" to="/JobseekerAssessments">
            <img src={Jobseeker_V} />
            &emsp;Jobseeker
          </p>
          {/* Employee link with toggle function */}
          <p onClick={questionToggle} to="/EmployeeAssessments" id="sidebar-item">
            <img src={Employee_V} />
          ...
          <div className={click ? "emp hidden" : "emp"} id="emp">
            ...
            <p onClick={() => handleParaClick('assessmentData')}
              className={activePara === 'assessmentData' ? 'active' : ''} id="sidebar-item" to="/EmployeeAssessments">Assessment Data</p>
            {/* Manage Employee link */}
            <p onClick={() => handleParaClick("manageemployee")}
              className={activePara === "manageemployee" ? "active" : ""} id="sidebar-item" to="/ManageEmployee">Manage Employee</p>
            {/* Employee Request */}
            <p onClick={() => handleParaClick('employeeRequest')}
              className={activePara === 'employeeRequest' ? 'active' : ''} id="sidebar-item" to="/EmployeeRequest">Employee Request</p>
          </div>
          {/* Settings link */}
          <p onClick={() => handleParaClick("settings")}
            className={activePara === "settings" ? "active" : ""} id="sidebar-item" to="/Settings">
            <img src={Settings_V} />
            &emsp;Settings
          </p>
          {/* Logo and logout section */}
          <div className="l-logo">
            <img src={L_Logo} className="logo-L" alt="Kanini Logo" width="190px" height="240px" />
            {/* Logout link */}
            <p onClick={() => handleParaClick("logout")}
              className={activePara === "logout" ? "active" : ""} id="sidebar-item" >
              <img src={Logout_V} />
              &emsp;Logout
            </p>
          </div>
          ...
          <div className="inline">
            <div className="hide-icon" id="hr">
              ...
              {getEmployee.length > 0 ? (
                <span className='bell-icon-cursor' onClick={RouteToRequest}><img src={Bell_V} id='bell' height='28px' width='28px' /><span><div className='dot-request'>&nbsp;</div></span></span>
              ) : (
                <img src={Bell_V} id='bell' height='28px' width='28px' />
              )}
              {/* Kanini Logo */}
              <span><img className='kanini-c' src={Kanini} alt='kanini' height='38px' width='38px' /></span>
