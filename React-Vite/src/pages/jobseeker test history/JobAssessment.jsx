...
import JobAvailableAssessment from '../jobseeker available assessment/JobSeekerAvailableAssessment';
...
const JobAssessment = () => {
  ...
  const handleOpen2 = (event, row, histId) => {
    ...
    setOpen2(true);
    setSelectedRow(row);
    console.log(' HistoryId',row);
    console.log('SelectedRowHistoryId',selectedRow);
    setHistId(histId);
    console.log('UserAssessmentId',histId);
  ...
  const handleAddChip = () => {
    if (inputValue.trim() !== '') {
      ...
      const selectedIds = selectedEmail.map((name) => {
          ...
          if (jobseeker) {
              console.log('if', jobseeker.empId)
...
const postEmail = async () => {
  ...
  return (
    <>
      <Navbar title="JobSeeker Assessment Data" desc="View completed details of all assessed and its corresponding details of jobseeker"></Navbar>
